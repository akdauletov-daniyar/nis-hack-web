    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <header className="mb-8 p-8 bg-dark rounded-3xl text-white relative overflow-hidden">
        <div className="relative z-10">
          <h1 className="text-3xl font-extrabold tracking-tight mb-2">{t('gov_title')}</h1>
          <p className="text-gray-400 font-medium">{t('gov_subtitle')}</p>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="bg-white p-8 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100">
          ...
          <form onSubmit={pullAnalytics} className="mb-8">
            <div className="flex flex-col sm:flex-row gap-4">
              <input value={zone} onChange={e=>setZone(e.target.value)} className="flex-1 px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-secondary focus:border-transparent outline-none transition" placeholder={t('gov_zonePlaceholder')}/>
              <button disabled={loading} type="submit" className="px-6 py-3 bg-secondary text-white font-bold rounded-xl hover:opacity-90 transition shadow-lg shadow-secondary/30 hover:-translate-y-0.5">
                {loading ? t('gov_analyzing') : t('gov_runAnalysis')}
              </button>
            </div>
          </form>
          ...
          {prediction && (
            <div className="p-6 border border-gray-100 bg-gray-50 rounded-2xl">
              <div className="flex justify-between items-start mb-4">
                ...
                <span className="px-3 py-1 bg-white border border-gray-200 text-dark font-mono text-xs font-bold rounded-lg shadow-sm">{t('gov_conf')}{prediction.confidence * 100}%</span>
              </div>
              ...
              <div className="bg-white p-4 rounded-xl border-l-4 border-secondary shadow-sm text-sm text-gray-800">
                <span className="font-bold text-secondary block mb-1">{t('gov_aiRec')}</span>
        ...
        <div className="bg-white p-8 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 relative overflow-hidden">
          ...
          <div className="space-y-8 mt-6">
            <div>
              <div className="flex justify-between text-sm mb-2 font-bold"><span className="text-gray-600">{t('gov_routesReported')}</span> <span className="text-dark">{routeCount.toLocaleString()}</span></div>
            ...
            <div>
              <div className="flex justify-between text-sm mb-2 font-bold"><span className="text-gray-600">{t('gov_volNetwork')}</span> <span className="text-dark">{volunteerCount}</span></div>
